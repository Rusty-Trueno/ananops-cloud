...
public class AnImcLogAspect {
    ...
    @Pointcut("@annotation(com.ananops.imc.aop.AnImcLog)")//定义切点
    ...
    private void handleLog(final JoinPoint joinPoint, final Object result) {
        ...
        try {
            ...
            AnImcLog relog = giveController(joinPoint);
            if(relog == null) {
                return ;
            }
     ...
     * 是否存在注解, 如果存在就记录日志
     */
    private static AnImcLog giveController(JoinPoint joinPoint) {
        Method[] methods = joinPoint.getTarget().getClass().getDeclaredMethods();
        String methodName = joinPoint.getSignature().getName();
        if (null != methods && 0 < methods.length) {
            for (Method met : methods) {
                AnImcLog relog = met.getAnnotation(AnImcLog.class);
