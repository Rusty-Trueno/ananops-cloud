package com.ananops.bmc.controller;

import com.ananops.bmc.domain.AnBmcBill;
import com.ananops.bmc.dto.BillCreateDto;
import com.ananops.bmc.dto.BillModifyAmountDto;
import com.ananops.bmc.dto.BillStatistics;
import com.ananops.bmc.dto.DeviceDto;
import com.ananops.bmc.service.impl.BaseServiceImpl;
...
//    PmcProjectFeignApi pmcProjectFeignApi;
    ...
    public R createNew(@ApiParam(name = "body",value="账单信息") @RequestBody BillCreateDto billCreateDto){
        // 判断projectId是否为空
        if (billCreateDto.getProjectId() == null){
            return R.error("创建工单时传入的参数projectId为空！getProjectId = "+billCreateDto.getProjectId());
        }
        //TODO:等待pmc完成之后取消注释，暂时使用默认值
...
//            return R.error("getProjectByProjectId获得的项目信息为空！projectId = "+billCreateDto.getProjectId());
//        }
//        if (pmcProjectDto.getContractId() == null){
//            return R.error("pmcProjectDto中的合同id为空！pmcProjectDto.getContractId() = "+pmcProjectDto.getContractId());
//        }
...
//            return R.error("通过getContractById获得的合同信息为空！ContractId = "+pmcProjectDto.getContractId());
//        }
...
//        }
    ...
    @ApiOperation(httpMethod = "POST",value = "创建账单")
    public R createFakeNew(@ApiParam(name = "body",value="账单信息") @RequestBody BillCreateDto billCreateDto){
...
//        }
        if (servicePrice != null) {
            return R.data(servicePrice.add(devicePrice));
        }
        return R.data(devicePrice);
    }
...
//        return pmcContractFeignApi.getContractById(contractId).getResult().getPaymentMoney();
    ...
    public R getStatistics(@ApiParam(name = "userId", value = "用户ID") @RequestParam Long userId,
    ...
    public  R getAllBillByUserId(@ApiParam(name = "userId",value = "用户id") @PathVariable Long userId){
        return R.data(baseServiceImpl.getAllBillByUserId(userId));
    }
    ...
    public R getBillsByUserIdAndState(@ApiParam(name = "userId",value = "用户id") @RequestParam Long userId,
                                                         @ApiParam(name = "state",value = "账单状态") @RequestParam String state){
        return R.data(baseServiceImpl.getBillsByUserIdAndState(userId,state));
    }
    ...
    public R getMoneySumByUserIdMonthAndLength(@ApiParam(name="userId",value="用户id") @RequestParam Long userId,
        ...
        return R.data(baseServiceImpl.getMoneySumByUserIdYearMonthAndLength(userId, year, month, length));
    }
    ...
    public R getMoneySumByUserIdAndMonth(@ApiParam(name="userId",value="用户id") @RequestParam Long userId,
                                                             @ApiParam(name="year",value="年份") @RequestParam int year,
                                                             @ApiParam(name="month",value="月份") @RequestParam int month){
        return R.data(baseServiceImpl.getMoneySumByUserIdYearAndMonth(userId, year, month));
    }

    @GetMapping(value = "/getBillNumByUserId/{userId}")
    @ApiOperation(httpMethod = "GET",value = "根据用户id获取账单数量")
    public R getBillNumByUserId(@ApiParam(name = "userId",value = "用户id") @RequestParam Long userId){
        return R.data(baseServiceImpl.getBillNumByUserId(userId));
    }

    @GetMapping(value = "/getBillNumByUserIdAndState/{userId}/{state}")
    @ApiOperation(httpMethod = "GET",value = "根据用户id和支付状态获取账单数量")
    public R getBillNumByUserIdAndState(@ApiParam(name = "userId",value = "用户id") @RequestParam Long userId,
                                                     @ApiParam(name = "state",value = "状态") @RequestParam String state){
        return R.data(baseServiceImpl.getBillNumByUserIdAndState(userId, state));
    }
    ...
    public R getBillNumByUserIdAndTransactionMethod(@ApiParam(name = "userId",value = "用户id") @RequestParam Long userId,
                                                       @ApiParam(name = "transactionMethod",value = "交易方式") @RequestParam String transactionMethod){
        return R.data(baseServiceImpl.getBillNumByUserIdAndTransactionMethod(userId, transactionMethod));
    }

    @GetMapping(value = "/getBillNumByUserIdAndAmount/{userId}/{amount}")
    @ApiOperation(httpMethod = "GET",value = "根据用户id和预约金额获取账单数量")
    public R getBillNumByUserIdAndAmount(@ApiParam(name = "userId",value = "用户id") @RequestParam Long userId,
                                                                   @ApiParam(name = "amount",value = "交易方式") @RequestParam BigDecimal amount){
        return R.data(baseServiceImpl.getBillNumByUserIdAndAmount(userId, amount));
    }

    @PostMapping(value = "/getBillById/{id}")
    @ApiOperation(httpMethod = "POST",value = "根据账单id获取账单")
    public  R getBillById(@PathVariable Long id){
        return R.data(baseServiceImpl.getOneBillById(id));
    }

    @GetMapping(value = "/getAmountByWorkOrder/{workOrderId}")
    @ApiOperation(httpMethod = "GET",value = "根据工单id获取金额")
    public R getAmountByWorkOrderId(@ApiParam(name = "workOrderId",value = "工单id") @RequestParam Long workOrderId){
        return R.data(baseServiceImpl.getAmountByWorkOrderId(workOrderId));
    }
    ...
    public void modifyAmount(@ApiParam(name = "modifyAmount",value = "修改金额") @RequestBody BillModifyAmountDto billModifyAmountDto){
        AnBmcBill bmcBill = baseServiceImpl.getBillById(billModifyAmountDto.getId());
    ...
    public R getAllUBillByState(@ApiParam(name = "userId",value = "用户id") @RequestParam Long userId,
                                                      @ApiParam(name = "state",value = "状态") @RequestParam String state){
        return R.data(baseServiceImpl.getAllUBillByState(userId, state));
    }

    @GetMapping(value = "/getBillByWorkOrderId/{workOrderId}")
    @ApiOperation(httpMethod = "GET",value = "根据状态及用户id获取账单")
    public R getBillByWorkOrderId(@ApiParam(name = "workOrderId",value = "工单id") @RequestParam Long workOrderId){
        return R.data(baseServiceImpl.getBillByWorkOrderId(workOrderId));
    }
}
